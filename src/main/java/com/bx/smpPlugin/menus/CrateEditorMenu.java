...
public class CrateEditorMenu extends BaseMenu {
    ...
    public CrateEditorMenu(SmpPlugin plugin, String crateId) {
    ...
    private ItemStack createEditorItem(CrateManager.CrateReward reward) {
        ...
        if (reward.grant().serializedItemData() != null && !reward.grant().serializedItemData().isBlank()) {
            try {
                item = com.bx.smpPlugin.utils.ItemSerializationUtils.deserialize(reward.grant().serializedItemData());
    ...
    private static int resolveSize(SmpPlugin plugin, String crateId) {
