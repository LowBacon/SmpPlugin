...
public class CrateEditorMenu extends BaseMenu {
    ...
    public CrateEditorMenu(FinnishSmp plugin, String crateId) {
    ...
    private ItemStack createEditorItem(CrateManager.CrateReward reward) {
        ...
        if (reward.grant().serializedItemData() != null && !reward.grant().serializedItemData().isBlank()) {
            try {
                item = com.bx.finnishSmp.utils.ItemSerializationUtils.deserialize(reward.grant().serializedItemData());
    ...
    private static int resolveSize(FinnishSmp plugin, String crateId) {
