...
import com.bx.smpPlugin.managers.BillfordManager;
...
public class BillfordMenu extends BaseMenu {
    ...
    public BillfordMenu(SmpPlugin plugin) {
    ...
    private static String resolveTitle(SmpPlugin plugin) {
    ...
    private static int resolveSize(SmpPlugin plugin) {
    ...
    public void build(Player player) {
        ...
        rewardPlaceholders.put(
                ...
                currentTrade.moneyBonus() > 0
                        ? "&7" + plugin.getCurrencyManager().singular(com.bx.smpPlugin.managers.CurrencyManager.CurrencyType.MONEY)
        ...
        rewardPlaceholders.put(
                ...
                currentTrade.shardBonus() > 0
                        ? "&7" + plugin.getCurrencyManager().singular(com.bx.smpPlugin.managers.CurrencyManager.CurrencyType.SHARDS)
            ...
            if (liveTrade.moneyBonus() > 0) {
                ...
                if (!depositResult.success()) {
                    fail(player, "BILLFORD.BUSY", "&cʙɪʟʟꜰᴏʀᴅ ᴄᴏᴜʟᴅ ɴᴏᴛ ᴄᴏᴍᴘʟᴇᴛᴇ ᴛʜᴇ "
                            + plugin.getCurrencyManager().singular(com.bx.smpPlugin.managers.CurrencyManager.CurrencyType.MONEY)
                            + " ʀᴇᴡᴀʀᴅ.");
                    return;
                }
            }
