...
import com.bx.finnishSmp.menus.AuctionHouseBrowseMenu;
import com.bx.finnishSmp.menus.PlayerAuctionGui;
import com.bx.finnishSmp.menus.SellGui;
...
public final class AuctionHouseCommand implements CommandExecutor, TabCompleter {
    ...
    public AuctionHouseCommand(FinnishSmp plugin) {
        this.plugin = plugin;
    }
    ...
    private void handleReload(Player player) {
        if (!PermissionUtils.has(player, "finnishsmp.admin.auctionhouse")) {
        ...
        manager.getPreferenceAsync(player.getUniqueId()).whenComplete((preference, throwable) -> {
            ...
            plugin.getSpigotScheduler().runEntity(player, () -> {
                    ...
                    boolean fastSell = preference.fastSellEnabled()
                            && hasEither(player, "finnishsmp.auctionhouse.fastsell", "donutauction.fastsell");
                    if (fastSell) {
                        com.bx.finnishSmp.models.AuctionCategory category;
                        boolean autoDetect = plugin.getConfigManager().getAuctionHouse()
                                .getBoolean("SETTINGS.AUTO_DETECT_CATEGORY", false);
                        if (autoDetect) {
                            category = com.bx.finnishSmp.models.AuctionCategory.findCategoryForItem(escrow);
                        } else {
                            category = com.bx.finnishSmp.models.AuctionCategory.from(preference.lastCategory());
                        }
    ...
    private void openBrowse(Player player) {
        ...
        if (autoFilter) {
            ...
            if (hand != null && hand.getType() != org.bukkit.Material.AIR) {
                category = com.bx.finnishSmp.models.AuctionCategory.findCategoryForItem(hand).name();
            }
        }
    ...
    private boolean canUse(Player player, String action) {
        return hasEither(
                player,
                "finnishsmp.auctionhouse." + action,
                "donutauction." + action
