...
import com.bx.finnishSmp.utils.PacketSidebarRenderer;
import com.bx.finnishSmp.utils.ScoreboardNumberHider;
...
public class ScoreboardManager {
    ...
    public ScoreboardManager(FinnishSmp plugin) {
        this.plugin = plugin;
    ...
    private void setupPlayerSpigot(Player player) {
        ...
        Objective obj = board.registerNewObjective("sidebar", Criteria.DUMMY,
                ColorUtils.toTitleCaseSmart(ColorUtils.toComponent("SmpPlugin")));
    ...
    private void updateTextSpigot(Player player, Scoreboard board, Objective obj) {
        obj.setDisplayName(getTitle(player));
    ...
    private String applySidebarEconomyPlaceholders(String line, Player player) {
        ...
        long playtimeSeconds = data != null ? data.getTotalPlaytimeSeconds() : 0L;
        ...
        String playtimeFormatted = formatPlaytime(playtimeSeconds);
        ...
        return line
                ...
                .replace("{money}", moneyShort)
                .replace("{kills}", String.valueOf(kills))
                .replace("{deaths}", String.valueOf(deaths))
                .replace("{playtime}", playtimeFormatted);
    }
    ...
    private String getTitle(Player player) {
        if (plugin.getLobbyManager() != null && plugin.getLobbyManager().isInLobby(player)) {
            return ColorUtils.toTitleCaseSmart(ColorUtils.colorize(plugin.getLobbyManager().getScoreboardTitle(), player));
        }
        if (plugin.getAFKLoungeManager() != null && plugin.getAFKLoungeManager().isPlayerInLounge(player)) {
            return ColorUtils.toTitleCaseSmart(ColorUtils.colorize(plugin.getAFKLoungeManager().getScoreboardTitle(), player));
        }
        List<String> titles = plugin.getConfigManager().getScoreboard().getStringList("SCOREBOARD.TITLE");
        if (titles.isEmpty()) {
            return ColorUtils.toTitleCaseSmart(ColorUtils.colorize("SmpPlugin", player));
        }
