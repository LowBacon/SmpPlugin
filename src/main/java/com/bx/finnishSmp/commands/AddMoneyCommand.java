...
public class AddMoneyCommand implements CommandExecutor {
    ...
    private static final String PERMISSION = "finnishsmp.admin.addmoney";

    private final FinnishSmp plugin;

    public AddMoneyCommand(FinnishSmp plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        var result = plugin.getEconomyManager().deposit(account, amount, com.bx.finnishSmp.models.EconomyReason.ADMIN_ADD);
        if (!result.success()) {
            sender.sendMessage(ColorUtils.toComponent(plugin.getConfigManager().getMessage("BALANCE.ADMIN.PLAYER-NOT-FOUND")));
            return true;
        }
