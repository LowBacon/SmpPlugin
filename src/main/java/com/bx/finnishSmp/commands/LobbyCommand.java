...
/**
 * Handles both /lobby [survival] and /northsmp &lt;lobby|survival&gt; (registered as the same
...
public final class LobbyCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> NORTHSMP_SUBCOMMANDS = List.of("lobby", "survival");
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        boolean isNorthSmpAlias = "northsmp".equalsIgnoreCase(label);

        if (isNorthSmpAlias) {
            if (args.length == 0) {
                sender.sendMessage(ColorUtils.toComponent("&cUsage: /northsmp <lobby|survival>"));
                return true;
            }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length != 1) {
            if (args.length == 2 && "border".equalsIgnoreCase(args[0]) && !"northsmp".equalsIgnoreCase(alias)) {
        ...
        List<String> options = "northsmp".equalsIgnoreCase(alias) ? NORTHSMP_SUBCOMMANDS : LOBBY_SUBCOMMANDS;
