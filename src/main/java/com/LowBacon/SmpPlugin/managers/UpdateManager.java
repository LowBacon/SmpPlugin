...
public class UpdateManager {
    ...
    private static final String VERSION_URL = "https://raw.githubusercontent.com/BeestoXd/FinnishSMP/main/version.txt";

    private final FinnishSmp plugin;
    private String latestVersion = null;
    private boolean updateAvailable = false;

    public UpdateManager(FinnishSmp plugin) {
        this.plugin = plugin;
    }
                    ...
                    if (version != null) {
                        ...
                        if (!version.isEmpty()) {
                            ...
                            if (this.updateAvailable) {
                                Bukkit.getConsoleSender().sendMessage(ColorUtils.colorize("&c[FinnishSmp] A new update is available: " + latestVersion + " (Current version: " + currentVersion + ")"));
                                Bukkit.getConsoleSender().sendMessage(ColorUtils.colorize("&c[FinnishSmp] Please download the latest version from: https://github.com/BeestoXd/FinnishSMP"));
                            } else {
                                plugin.getLogger().info("You are running the latest version of FinnishSmp (" + currentVersion + ").");
                            }
                        }
                    }
                }
