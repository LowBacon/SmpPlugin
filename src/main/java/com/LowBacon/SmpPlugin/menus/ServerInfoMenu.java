...
public class ServerInfoMenu extends BaseMenu {
    ...
    public ServerInfoMenu(FinnishSmp plugin) {
        this(plugin, 1);
    }

    public ServerInfoMenu(FinnishSmp plugin, int page) {
        this(plugin, page, loadPages(plugin));
    }

    private ServerInfoMenu(FinnishSmp plugin, int requestedPage, List<PageDefinition> loadedPages) {
    ...
    private static List<PageDefinition> loadPages(FinnishSmp plugin) {
    ...
    private static List<ButtonDefinition> loadButtons(FinnishSmp plugin, ConfigurationSection buttonsSection, String pageKey) {
    ...
    private static ButtonAction resolveAction(FinnishSmp plugin, String key, ConfigurationSection buttonSection) {
        ...
        return switch (key.toUpperCase(Locale.ROOT)) {
            ...
            case "ECONOMY" -> ButtonAction.info(List.of(
                    "&7ᴇᴀʀɴ " + plugin.getCurrencyManager().plural(com.bx.finnishSmp.managers.CurrencyManager.CurrencyType.MONEY)
    ...
    private static String configuredTitle(FinnishSmp plugin, int requestedPage, List<PageDefinition> loadedPages) {
        if (!loadedPages.isEmpty()) {
    ...
    private static int configuredSize(FinnishSmp plugin, int requestedPage, List<PageDefinition> loadedPages) {
        if (!loadedPages.isEmpty()) {
            int safePage = Math.max(1, Math.min(requestedPage, loadedPages.size()));
    ...
    private static PageDefinition createDefaultGettingStartedPage(FinnishSmp plugin) {
        String moneyPlural = plugin.getCurrencyManager().plural(com.bx.finnishSmp.managers.CurrencyManager.CurrencyType.MONEY);
    ...
    private static PageDefinition createDefaultCommandsPage(FinnishSmp plugin) {
        String moneyPlural = plugin.getCurrencyManager().plural(com.bx.finnishSmp.managers.CurrencyManager.CurrencyType.MONEY);
        return new PageDefinition(
                "3",
                "&8ᴜѕᴇꜰᴜʟ ᴄᴏᴍᴍᴀɴᴅѕ",
