...
public final class OrdersBedrockManager {
    ...
    public OrdersBedrockManager(SmpPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean isBedrockPlayer(Player player) {
        ...
        try {
            return floodgateIsBedrock(player);
    ...
    private boolean send(Player player, Form form) {
        try {
            return floodgateSendForm(player, form);
        } catch (Throwable throwable) {
    ...
    /**
     * Uses reflection for the optional Floodgate API. This keeps Floodgate as a
     * true runtime integration instead of making the whole build depend on
     * OpenCollab's Maven repository. Floodgate is optional at runtime anyway.
     */
    private boolean floodgateIsBedrock(Player player) {
        try {
    ...
    private boolean floodgateSendForm(Player player, Form form) {
        try {
            Class<?> apiClass = Class.forName("org.geysermc.floodgate.api.FloodgateApi");
            Object api = apiClass.getMethod("getInstance").invoke(null);
            ...
            for (java.lang.reflect.Method method : apiClass.getMethods()) {
                ...
                if (!parameters[0].isAssignableFrom(java.util.UUID.class)
                        && !java.util.UUID.class.isAssignableFrom(parameters[0])) {
                    continue;
                }

                if (!parameters[1].isAssignableFrom(form.getClass())
                        && !parameters[1].isInstance(form)) {
                    continue;
                }

                Object result = method.invoke(api, player.getUniqueId(), form);
                return !(result instanceof Boolean) || (Boolean) result;
            }
        } catch (ReflectiveOperationException | LinkageError ignored) {
            // Floodgate is optional. Fall back to the normal Java GUI.
        }
        return false;
    }
}
