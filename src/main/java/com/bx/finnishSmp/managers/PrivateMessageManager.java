...
public class PrivateMessageManager {
    ...
    private static final String BYPASS_DISABLED_PERMISSION = "finnishsmp.message.bypass-disabled";

    private final FinnishSmp plugin;
    private final Map<UUID, UUID> replyTargets = new ConcurrentHashMap<>();

    public PrivateMessageManager(FinnishSmp plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean sendPrivateMessage(CommandSender sender, Player target, String message) {
        ...
        if (sender instanceof Player player) {
            ...
            if (targetData != null && !PermissionUtils.has(sender, BYPASS_DISABLED_PERMISSION)) {
                com.bx.finnishSmp.models.ThreeChoice choice = targetData.getPrivateMessagesChoice();
