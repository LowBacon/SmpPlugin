...
public final class PlayerSettingsMenu extends BaseMenu {
    ...
    public PlayerSettingsMenu(SmpPlugin plugin) {
        super(
                plugin,
    ...
    public void handleClick(int slot, Player player) {
        ...
        switch (key) {
            ...
            case "NIGHT_VISION" -> {
                boolean enabled = com.bx.smpPlugin.utils.NightVisionUtils.toggle(plugin, player);
        ...
        return switch (key) {
            ...
            case "DISABLE_MOB_SPAWN" -> {
                ...
                if (disabled && data.getMobSpawnDisabledUntil() > 0) {
                    ...
                    if (remainingSecs > 0) {
                        yield new ButtonState("&aEnabled &7(" + com.bx.smpPlugin.utils.NumberUtils.formatTime(remainingSecs) + " left)", true);
                    }
                }
            ...
            case "NIGHT_VISION" -> state(com.bx.smpPlugin.utils.NightVisionUtils.isEnabled(plugin, player));
    ...
    private ButtonState quickBuyState(Player player) {
        if (!PermissionUtils.has(player, "smpplugin.auctionhouse.fastbuy")
    ...
    private ButtonState quickSellState(Player player) {
        if (!PermissionUtils.has(player, "smpplugin.auctionhouse.fastsell")
