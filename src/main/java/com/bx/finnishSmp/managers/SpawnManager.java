...
public class SpawnManager {
    ...
    public SpawnManager(FinnishSmp plugin) {
        this.plugin = plugin;
    }
    ...
    /**
     * Saves one primary spawn point without requiring a spawn-menu area.
     * This is used by /setspawn for straightforward single-spawn servers.
     */
    public boolean setPrimarySpawnLocation(Location location) {
        if (location == null || location.getWorld() == null) {
            return false;
        }

        FileConfiguration config = plugin.getConfigManager().getConfig();
        config.set("LOCATIONS.SPAWN-LOCATION", LocationUtils.serialize(location));
        ...
        try {
            ...
            plugin.getLogger().warning("[SpawnManager] failed to save the primary spawn location: " + exception.getMessage());
            return false;
        }
    ...
    public Location resolveCommandDestination(AreaType type) {
        ...
        if (!isMenuEnabled(type)) {
            return legacyLocation != null ? legacyLocation : resolveBoundCuboidDestination(type);
        }
