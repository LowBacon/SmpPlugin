...
public final class LobbyListener implements Listener {
    ...
    public LobbyListener(FinnishSmp plugin) {
        this.plugin = plugin;
    }
    ...
    public void onJoin(PlayerJoinEvent event) {
        ...
        if (plugin.getMaintenanceManager() != null && plugin.getMaintenanceManager().isMaintenanceActive()) {
            String bypassPerm = plugin.getConfigManager().getNetwork()
                    .getString("MAINTENANCE.BYPASS_PERMISSION", "FINNISHSMP.ADMIN.MAINTENANCE.BYPASS");
    ...
    private boolean bypassesBorder(Player player) {
        return player.hasPermission("finnishsmp.bypass.lobby") || player.hasPermission("finnishsmp.bypass.lobby.border");
    }

    // ------------------------------------------------------------------
    // Protection - all gated by lobby.yml's PROTECTION section and lobbyManager.isInLobby(),
    // and skipped entirely for finnishsmp.bypass.lobby.
    // ------------------------------------------------------------------

    private boolean protectedFor(Player player) {
        return !player.hasPermission("finnishsmp.bypass.lobby") && plugin.getLobbyManager().isInLobby(player);
    }
    ...
    public void onIgnite(BlockIgniteEvent event) {
        ...
        if (event.getPlayer() != null && event.getPlayer().hasPermission("finnishsmp.bypass.lobby")) return;
        event.setCancelled(true);
    }
    ...
    public void onHangingBreak(HangingBreakEvent event) {
        ...
        if (event instanceof HangingBreakByEntityEvent byEntity && byEntity.getRemover() instanceof Player p
                && p.hasPermission("finnishsmp.bypass.lobby")) {
            return;
        }
        event.setCancelled(true);
    }
