...
import com.bx.finnishSmp.models.AuctionBrowseEngine;
...
import com.bx.finnishSmp.models.PlayerAuctionSession;
import com.bx.finnishSmp.models.PlayerPreference;
import com.bx.finnishSmp.storage.AuctionHouseRepository;
...
public final class AuctionHouseManager {
    ...
    public AuctionHouseManager(FinnishSmp plugin) {
        this.plugin = plugin;
    ...
    public int getMaxActiveListings(Player player) {
        ...
        resolved = Math.max(resolved, PermissionUtils.resolveHighestExactNumberedPermission(
                player,
                "finnishsmp.auctionhouse.",
                MAX_PERMISSION_VALUE
        ));
        resolved = Math.max(resolved, PermissionUtils.resolveHighestExactNumberedPermission(
                player,
                "finnishsmp.auctionhouse.limit.",
                MAX_PERMISSION_VALUE
        ));
        resolved = Math.max(resolved, PermissionUtils.resolveHighestExactNumberedPermission(
                player,
    ...
    private boolean hasPermission(Player player, String action) {
        return PermissionUtils.has(player, "finnishsmp.auctionhouse." + action)
                || PermissionUtils.has(player, "donutauction." + action)
                || PermissionUtils.has(player, "finnishsmp.admin.auctionhouse");
    }
